...
# Elements that conventionally come last in a formula (electronegative, in
# decreasing electropositivity so e.g. SiO2 / TiO2 / CaCO3 read naturally).
_NONMETAL_ORDER = ['B', 'C', 'Si', 'Ge', 'N', 'P', 'As', 'Sb', 'H',
                   'Te', 'Se', 'S', 'At', 'I', 'Br', 'Cl', 'F', 'O']
...
def extract_formula_from_d12(d12_file: Path) -> Optional[str]:
    ...
    # Atom line: atomic number followed by three coordinates. Coordinates may be
    # in scientific notation (e.g. "4.470427420056E-18"), which the old
    # "[\d.-]+" pattern could not match — it stopped at the 'E', failed the
    # line, and fell through to a whole-file scan that parsed basis-set rows as
    # atoms. The float sub-pattern below accepts the exponent form.
    _float = r'[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?'
    atom_pattern = r'^\s*(\d+)\s+' + _float + r'\s+' + _float + r'\s+' + _float

    def _symbol(z: int) -> Optional[str]:
        # CRYSTAL flags pseudopotential atoms as Z + 200 (e.g. "247" == Ag).
        if z > 200:
            z -= 200
    ...
    for line in lines:
        ...
        if in_geometry and line and not line.startswith('#'):
            ...
            if match:
                element = _symbol(int(match.group(1)))
                if element:
                    atoms.append(element)

    # Fallback: scan only the geometry block (everything before the first END,
    # which precedes the basis-set section) so basis rows are never mistaken for
    # atoms.
    if not atoms:
        geometry_block = re.split(r'\n\s*END\b', content, maxsplit=1)[0]
        for atomic_num_str in re.findall(atom_pattern, geometry_block, re.MULTILINE):
            element = _symbol(int(atomic_num_str))
            if element:
                atoms.append(element)
    ...
    # Count atoms and create formula. Convention: electropositive elements
    # (cations/metals) first, ordered by atomic number, then the nonmetal anions
    # in a fixed electronegativity order (so SiO2, TiPbO3, AgBr read naturally).
    atom_counts = Counter(atoms)
    symbol_to_z = {sym: z for z, sym in _ELEMENT_SYMBOLS.items()}

    cations = sorted(
        (el for el in atom_counts if el not in _NONMETAL_ORDER),
        key=lambda el: symbol_to_z.get(el, 999)
    )
    anions = [el for el in _NONMETAL_ORDER if el in atom_counts]

    formula_parts = []
    for element in cations + anions:
