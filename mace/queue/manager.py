...
class EnhancedCrystalQueueManager:
    ...
    def handle_completed_calculation(self, calc_id: str):
        ...
        # Plan next calculation in workflow — workflow runs only. A manual
        # `mace submit` reaches this callback too, and progressing it built a
        # whole workflow (and workflow directory) out of one hand-run job.
        if self.workflow_enabled and self.auto_submit_followups and self._belongs_to_workflow(calc):
    ...
    def _belongs_to_workflow(self, calc: Dict) -> bool:
        """Whether ``calc`` was submitted as part of a planned workflow.

        Two independent signals, either of which is authoritative:
        ``workflow_id`` in the calculation's settings (written by the executor
        and by `mace submit --progress`), or ``MACE_WORKFLOW_ID`` exported into
        the job script by the engine. A hand-run `mace submit` has neither, and
        must not progress — see `_planless_progression_allowed` in the engine.

        Note this deliberately does NOT consult ``is_workflow_context``: that
        is a directory heuristic (it walks up to 7 parents looking for
        ``workflow_*`` dirs), so a manual run inside — or merely below — a tree
        that once held a workflow would otherwise be adopted into it.
        """
        if os.environ.get('MACE_WORKFLOW_ID'):
            return True
        try:
            ...
            settings = {}
        if settings.get('workflow_id'):
            return True
        metadata = calc.get('metadata') or {}
        return bool(isinstance(metadata, dict) and metadata.get('workflow_id'))
    ...
    def _run_callback_check_locked(self, mode='completion'):
        ...
        if mode == 'completion':
            ...
            if self.is_workflow_context and self.workflow_enabled:
                ...
                # Manual run: record status and stop. This used to fall through
                # to process_new_d12_files(), which globs **/*.d12 and submits
                # every deck with no DB record — so the SP/D3 the user had just
                # generated by hand got submitted the moment an unrelated job
                # finished. Keeping the queue fed is what `mace manager` and
                # `--callback-mode submit_new` are for; both still do it.
                print("  Manual submission (no workflow plan) - status recorded, "
                      "no follow-up jobs submitted.")
