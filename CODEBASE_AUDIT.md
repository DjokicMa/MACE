...
7. **MAJOR** ✅ FIXED `materials.py:635-638` — `cleanup_old_records` now uses `datetime.now() - timedelta(days=days_old)` (timedelta imported L15); smoke-verified it runs without ValueError. (Was `replace(day=day-30)`.)
...
11. **MINOR** ❌ NOT A BUG (verified 2026-06-13, b/deb705f0..) `analysis/aggregation.py:149, 185` — claim was that `group_by='conductivity_type'/'energy_range'` (and `atoms_range`) look up property names the extractor never writes. FALSE: verified against real `test/` SP output that the extractor emits `conductivity_type` (=`non_metallic`), `total_energy_au`, `atoms_in_unit_cell`, and `band_gap` — grouping works. Locked by `tests/test_aggregation_keys.py`. (Recurring overstated-audit pattern.)
12. **MINOR** ✅ FIXED (`b02cc0d5`) `materials_contextual.py` — `copy_to_context` passed already-encoded JSON strings into `create_material`/`create_calculation` (which `json.dumps` again → double-encoded). Now JSON-decoded via `_loads_or_none` before re-passing. Also fixed the same method's `create_or_update_material` typo and `unit=`/`conditions=` kwargs. Tests: `tests/test_contextual_db.py`.
...
3. **MAJOR** ✅ FIXED (7f21c785) `d3_interactive.py:575-607` — interactive path passes descriptive strings ('cubic_fc') to a function matching single letters → FCC treated as simple cubic, rhombohedral as hexagonal; wrong-family paths in single-file mode. (Removed the redundant/corrupting conversion; lattice_type stays the letter. Verified SG225/229/166 now correct.)
...
14. **MINOR** ⏭ N/A `d3_interactive.py:1399` + `CRYSTALOptToD3.py:909-911` — POTC n_points double-negated (positive NPU emitted, CRYSTAL then expects inline coords that aren't there); atoms list collected but never written. (Not in the user's workflow — CHARGE+POTENTIAL calcs use ECH3/POT3 3D grids, not POTC; 0 POTC files in test/. Left as-is.)
