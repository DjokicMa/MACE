#!/bin/bash --login
#SBATCH -J NiCltryCOF102_sym_BULK_OPTGEOM_DZ_optimized_optimized
#SBATCH -o NiCltryCOF102_sym_BULK_OPTGEOM_DZ_optimized_optimized-%J.o
...
export JOB=NiCltryCOF102_sym_BULK_OPTGEOM_DZ_optimized_optimized
...
if [ -f $DIR/enhanced_queue_manager.py ]; then
    cd $DIR
    python enhanced_queue_manager.py --status-file crystal_job_status.json --max-jobs 250 --reserve 30 --max-submit 5 --callback-mode completion --max-recovery-attempts 3
    ...
    cd $DIR
    ./crystal_queue_manager.py --status-file crystal_job_status.json --max-jobs 250 --reserve 30 --max-submit 5
elif [ -f $DIR/../../../../crystal_queue_manager.py ]; then
    cd $DIR/../../../../
