...
Run under pytest (collected as ``test_back_integration``) or standalone:
    python tests/test_back_integration.py
"""
import sys
from pathlib import Path

# Make Crystal_d12/ importable whether run via pytest or standalone (python tests/test_back_integration.py).
...
def _run_checks():
    """Drive every integration check; failures accumulate in the module-level ``_fail``."""
    _fail.clear()
...
def test_back_integration():
    """pytest entry point: run every back-navigation check, assert none failed."""
    _run_checks()
    assert not _fail, f"back-integration failures: {_fail}"
